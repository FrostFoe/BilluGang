...
def main():
    ...
    parser.add_argument(
        "-c", "--clear", help="Delete output folder", action="store_true"
    )
...
def generate_movie_with_audio(project_name, output_folder):
    ...
    video.write_videofile(
        os.path.join(output_folder, f"{project_name}.mp4"),
        codec="libx264",
        audio_codec="aac",
    )
...
def generate_image(block_number, user, time_of_message, lines, output_folder):
    ...
    for i, line in enumerate(lines):
        line = line.split("-->")[0]
        if line[0] == "@":
            generate_line_with_mention(
                message_position, template, i, line, full_mention
            )
            message_position[1] += MESSAGE_DY
        elif line.startswith("[image:"):
            message_position = generate_imported_image(
                message_position, template, i, line
            )
    ...
    if full_mention:
        draw.rectangle(
            (0, 0, LINE_MENTION_WIDTH, message_position[1] + (MESSAGE_DY * 0.25)),
...
def generate_imported_image(message_position, template, i, line):
    ...
    draw_image.rounded_rectangle(
        [(mask_radius, 0), (image.width - mask_radius, image.height)],
        radius=mask_radius,
        fill=255,
    )
...
def generate_line_with_mention(message_position, template, i, line, full_mention):
    ...
    draw.rectangle(
        (
            ...
            message_position[1],
            IMAGE_WIDTH,
            message_position[1] + (MESSAGE_DY * 0.85),
        ),
    ...
    user_mention_position = (
        text_x,
        text_y + 10,
        text_x + user_text_width,
        text_y + user_text_height + 12,
    )
...
def generate_profile_picture_name_time(template, user, time_of_message):
    ...
    profile_picture.thumbnail(
        [sys.maxsize, PROFILE_PIC_WIDTH], Image.Resampling.LANCZOS
    )
    ...
    draw_time.text(
        time_position, f"Today at {time_of_message}", TIME_FONT_COLOR, font=TIME_FONT
    )
